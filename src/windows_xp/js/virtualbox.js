export function initVirtualBox(win, showNotification) {
    ...
    { 
      ...
      getUrl: () => {
        ...
        const params = new URLSearchParams({
          sessionStartTime: analytics.sessionStartTime || Date.now(),
          clickCount: analytics.clickCount || 0
        });
        return `/windows-xp/?${params.toString()}`;
      }
    },
  ...
  function startVM(vm) {
    ...
    if (vm.name === 'Windows XP') {
      // Load Windows XP with analytics state in URL
      window.location.href = vm.getUrl();
      return;
    }
    
    // For other VMs, use iframe as before
